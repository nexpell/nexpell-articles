...
if (!function_exists('nx_articles_current_lang')) {
    function nx_articles_current_lang(): string
    {
        global $languageService;

        $currentLang = '';
        if (isset($GLOBALS['lang'])) {
            $currentLang = (string)$GLOBALS['lang'];
        }
        if ($currentLang === '' && isset($languageService) && is_object($languageService) && method_exists($languageService, 'detectLanguage')) {
            $currentLang = (string)$languageService->detectLanguage();
        }
        if ($currentLang === '') {
            $currentLang = (string)($_SESSION['language'] ?? 'de');
        }

        $currentLang = substr(strtolower($currentLang), 0, 2);
        return preg_match('/^[a-z]{2}$/', $currentLang) ? $currentLang : 'de';
    }
}

$lang = nx_articles_current_lang();
...
if ($action == "show" && isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if (mysqli_num_rows($ergebnis)) {
        ...
        while ($ds = mysqli_fetch_array($ergebnis)) {
            ...
            // Übersetzung laden
            $translate = new multiLanguage(nx_articles_current_lang());
